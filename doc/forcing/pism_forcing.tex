...
Over ice-covered grid cells, daily variability can also be parameterized as a linear function of near-surface air temperature $\sigma = a + b*T$ or a quadratic function $\sigma = a + b*T + c*T^2$ using the \config{pdd_std_dev_method}. FIXME. The corresponding parameters \config{pdd_std_dev_param_a} and \config{pdd_std_dev_param_b}. This parametrization replaces prescribed standard deviation values over glacierized grid cells as defined by the \variable{mask} variable (see \config{geometry.ice_free_thickness_standard}). Default values were derived from the ERA-40 reanalysis \cite{SeguinotRogozhina2014} and from in-situ observation \cite{Wake2015} over the Greenland ice sheet.
